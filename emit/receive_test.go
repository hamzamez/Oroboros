...
func receiveTarget(t *testing.T) *Target {
	t.Helper()
	tg, err := LoadTarget("../targets/go")
	if err != nil {
		t.Fatal(err)
	}
	tg.Prims["x.decode"] = Prim{Name: "x.decode", Args: []string{"string"},
		Results: []string{"int -2147483648 2147483647", "int"},
		Kind:    "expr", Form: "utf8.DecodeRuneInString(%s)", Import: "unicode/utf8"}
	tg.Prims["x.rev8"] = Prim{Name: "x.rev8", Args: []string{"int 0 255"},
		Result: "int 0 255", Kind: "expr", Form: "bits.Reverse8(uint8(%s))", Import: "math/bits"}
	tg.Prims["x.count"] = Prim{Name: "x.count", Args: []string{"string"},
...
func TestSeveralRangeResultsAreReceivedAsInt(t *testing.T) {
	...
	got, err := Func(tg, "f", strSig("s"), term)
	if err != nil {
		t.Fatal(err)
	}
...
func TestASingleRangeResultIsReceivedAsInt(t *testing.T) {
	...
	got, err := Func(tg, "f", &core.Sig{Params: []core.SigParam{{Name: "x", Type: "int 0 255"}}, Result: "int"}, term)
	if err != nil {
		t.Fatal(err)
	}
	...
	got, err = Func(tg, "g", strSig("s"), plain)
	if err != nil {
		t.Fatal(err)
	}
