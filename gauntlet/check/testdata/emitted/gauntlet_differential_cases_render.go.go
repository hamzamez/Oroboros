...
func GenRun(n int) string {
	...
	if (x.Cmp((big.NewInt(int64(0)))) == 0) {
		...
		for ; ; i2 = (i2 + 1) {
			...
			nv1917 := (func(oroZ *big.Int, oroK int) *big.Int { if oroZ.BitLen() > oroK { panic("bignum overflow: the declared range is too small") }; return oroZ }((new(big.Int).Quo(v, (big.NewInt(int64(100000000))))), 201))
			s2 := ""
			var k int = 0
